...
def main(args):
    ...
    en = Ensemble(filenames = args.ensemble_files, filepath=args.result_path)
    ...
    files_title = datetime.datetime.now() # 이름 너무 길것 같아서 바꿈
...
if __name__ == "__main__":
    ...
    이 때, 경로(submit)와 확장자(.csv)는 입력해야합니다!!!!!
    ...
    arg("--ensemble_files", nargs='+',required=True,
        ...
        help='required: 앙상블할 submit 파일 경로를 쉼표(,)로 구분하여 모두 입력해 주세요. 이 때, .csv와 같은 확장자는 입력하지 않습니다.')
    ...
    arg('--result_path',type=str, default='./outputs/',
        help='optional: 앙상블할 파일을 저장할 경로를 전달합니다. (default:"./outputs/")')
